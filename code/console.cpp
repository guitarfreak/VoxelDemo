...
void Console::update(Input* input, Vec2 currentRes, int fontHeight, float dt) {
	...
	if(isActive) {
		...
		dcTextSelection(inputBuffer, cs.inputFont, inputStartPos, cursorIndex, markerIndex, cs.selectionColor, vec2i(-1,0));
			...
			for(int i = 0; i < mainBufferSize; i++) {
				...
				if(!textOutside) {
					...
					if(i == bodySelectionIndex) {
						...
						printf("%i %i\n", bodySelectionMarker1, bodySelectionMarker2);
						dcTextSelection(mainBuffer[i], cs.bodyFont, textPos, bodySelectionMarker1, bodySelectionMarker2, cs.selectionColor, vec2i(-1,1), wrappingWidth);
					}
