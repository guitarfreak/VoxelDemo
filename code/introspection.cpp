...
MEMBER_ARRAY(CURRENT_TYPE) = {
	...
	MEMBER_INFO( ParticleEmitter, emitter ),
	MEMBER_INFO( uchar, blockType ),
	MEMBER_INFO( bool, exploded ),
};
...
bool TypeTraverse::next() {
	...
	TypeTraverseStackElement* e = stack + stackCount-1;
	StructInfo* structInfo = getStructInfo(e->type);

	isArray = e->isArray;

	lastActionWasPush = action == TRAVERSETYPE_PUSH;

	int maxCount = isArray ? e->arraySize : structInfo->memberCount;
	if(e->index < maxCount) {

		StructMemberInfo* structMemberInfo = structInfo->list + e->index;
