...
Rect scissorRectScreenSpace(Rect r, float screenHeight) {
	return {r.min.x, r.min.y+screenHeight, r.max.x, r.max.y+screenHeight};
}

void guiScissorTestScreen(Rect r) {
	Rect sr = scissorRectScreenSpace(r, theGraphicsState->screenRes.h);
	if(sr.w() < 0 || sr.h() < 0) sr = rect(0,0,0,0);

	int left   = roundIntf(sr.left);
	int bottom = roundIntf(sr.bottom);
	int right  = roundIntf(sr.right);
	int top    = roundIntf(sr.top);
...
void guiSetScissor(bool enable) {
	if(enable) dcEnable(STATE_SCISSOR);
...
void guiDrawTextSelection(char* text, Font* font, Vec2 startPos, int index1, int index2, Vec4 color, Vec2i align = vec2i(-1,1), int wrapWidth = 0) {
	dcTextSelection(text, font, startPos, index1, index2, color, align, wrapWidth);
}

void guiDrawLineH(Vec2 p0, Vec2 p1, Vec4 color, bool roundUp = false) {
	float off = roundUp?0.5f:-0.5f;
	guiDrawLine(round(p0)+vec2(0, off), 
	            round(p1)+vec2(0, off), color);
}
void guiDrawLineV(Vec2 p0, Vec2 p1, Vec4 color, bool roundUp = false) {
	float off = roundUp?0.5f:-0.5f;
	guiDrawLine(round(p0)+vec2(off, 0), 
                round(p1)+vec2(off, 0), color);
}
